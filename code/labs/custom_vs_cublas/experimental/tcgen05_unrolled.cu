/**
 * Stage 11: Unrolled tcgen05 GEMM
 * ================================
 * 
 * Explicitly unroll the K-block inner loop using CUTE_UNROLL.
...
gemm_unrolled(ATensor mA, BTensor mB, CTensor mC, DTensor mD,
  ...
  // Mainloop - same as original but using CUTE_UNROLL hint
  for (int k = 0; k < num_k_tiles; ++k) {
    ...
    if (elect_one_warp) {
      ...
      // Use CUTE_UNROLL to encourage compiler to unroll
      CUTE_UNROLL
...
torch::Tensor run_unrolled_matmul(torch::Tensor a, torch::Tensor b) {
  ...
  using SharedStorageT = UnrolledSharedStorage<TypeA, TypeB, decltype(sA_layout), decltype(sB_layout)>;
...
torch::Tensor matmul_tcgen05_unrolled(torch::Tensor a, torch::Tensor b) {
  return unrolled_impl::run_unrolled_matmul(a, b);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("matmul_tcgen05_unrolled", &matmul_tcgen05_unrolled);
}
