/**
 * Stage 7: True Warp-Specialized tcgen05 GEMM (4-stage pipeline)
 * ==============================================================
 * 
 * This kernel uses a 4-stage deep pipeline with proper prologue/mainloop
 * structure based on CUTLASS patterns for optimal overlap.
 * 
 * Key optimization: Fill pipeline in prologue, then steady-state mainloop.
 */
...
// Helper macro for TMA load into specific stage
#define DO_TMA_LOAD(storage, stage, k_idx, tma_atom_A, tma_atom_B, tma_transaction_bytes, tCgCoordA, tCgCoordB) \
  do { \
    auto sA_s = storage.tensor_sA(stage); \
    auto sB_s = storage.tensor_sB(stage); \
    auto [tAgA_s, tAsA_s] = tma_partition(tma_atom_A, Int<0>{}, Layout<_1>{}, \
        group_modes<0,3>(sA_s), group_modes<0,3>(tCgCoordA)); \
    auto [tBgB_s, tBsB_s] = tma_partition(tma_atom_B, Int<0>{}, Layout<_1>{}, \
        group_modes<0,3>(sB_s), group_modes<0,3>(tCgCoordB)); \
    cute::set_barrier_transaction_bytes(storage.tma_barrier[stage], tma_transaction_bytes); \
    copy(tma_atom_A.with(storage.tma_barrier[stage]), tAgA_s(_, k_idx), tAsA_s); \
    copy(tma_atom_B.with(storage.tma_barrier[stage]), tBgB_s(_, k_idx), tBsB_s); \
  } while(0)
...
gemm_warp_spec_4stage(ATensor mA, BTensor mB, CTensor mC, DTensor mD,
  ...
  // All 4 stages smem tensors and fragments
  ...
  // TMA partitions for stage 0 (used to get transaction size)
  ...
  // ==========================================================================
  // PROLOGUE: Fill pipeline with first 3 tiles (stages 0, 1, 2)
  // ==========================================================================
  
  if (num_k_tiles > 0 && elect_one_warp && elect_one_thr) {
    DO_TMA_LOAD(storage, 0, 0, tma_atom_A, tma_atom_B, tma_transaction_bytes, tCgCoordA, tCgCoordB);
  }
  if (num_k_tiles > 1 && elect_one_warp && elect_one_thr) {
    DO_TMA_LOAD(storage, 1, 1, tma_atom_A, tma_atom_B, tma_transaction_bytes, tCgCoordA, tCgCoordB);
  }
  if (num_k_tiles > 2 && elect_one_warp && elect_one_thr) {
    DO_TMA_LOAD(storage, 2, 2, tma_atom_A, tma_atom_B, tma_transaction_bytes, tCgCoordA, tCgCoordB);
  }

  // ==========================================================================
  // MAIN LOOP: Compute tile K, load tile K+3
  // ==========================================================================
  
  for (int k_tile = 0; k_tile < num_k_tiles; ++k_tile) {
    int curr = k_tile % kStages;
    int next_k = k_tile + 3;  // 3 tiles ahead (prologue filled 0,1,2)
    ...
    // Issue load for tile K+3 (runs parallel with compute below)
    if (next_k < num_k_tiles && elect_one_warp && elect_one_thr) {
      DO_TMA_LOAD(storage, next_stage, next_k, tma_atom_A, tma_atom_B, tma_transaction_bytes, tCgCoordA, tCgCoordB);
    }
...
torch::Tensor run_warp_spec_matmul(torch::Tensor a, torch::Tensor b) {
  ...
  using SharedStorageT = WarpSpecSharedStorage<TypeA, TypeB, decltype(sA_layout), decltype(sB_layout)>;
  ...
  auto* kernel_ptr = &gemm_warp_spec_4stage<
...
torch::Tensor matmul_tcgen05_warp_spec(torch::Tensor a, torch::Tensor b) {
  return warp_spec_impl::run_warp_spec_matmul(a, b);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("matmul_tcgen05_warp_spec", &matmul_tcgen05_warp_spec);
}
