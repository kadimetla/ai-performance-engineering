"""Full-stack lab scenarios composed from chapter benchmarks.
...
@dataclass(frozen=True)
class ScenarioSpec:
    """Full-stack scenario definition with baseline/optimized phases."""
...
SCENARIO_SPECS: Dict[str, ScenarioSpec] = {
    ...
    "03_moe_cuda": ScenarioSpec(
        key="03_moe_cuda",
        title="MoE CUDA Kernels",
        summary="MoE CUDA tuning suite: routing, KV transfer, and decode kernels",
        chapters=("labs/moe_cuda",),
        baseline_phases=(
            _phase(
                "moe_cuda_router_dense",
                "labs.moe_cuda.baseline_router",
                "labs/moe_cuda",
                "Dense MoE router baseline",
            ),
            _phase(
                "moe_cuda_kv_transfer",
                "labs.moe_cuda.baseline_kv_transfer",
                "labs/moe_cuda",
                "Sequential KV transfers between prefill and decode",
            ),
            _phase(
                "moe_cuda_decode_attention",
                "labs.moe_cuda.baseline_decode_attention",
                "labs/moe_cuda",
                "Naive decode attention baseline",
            ),
            _phase(
                "moe_cuda_decode_kernel_baseline",
                "labs.moe_cuda.baseline_decode_kernel",
                "labs/moe_cuda",
                "Naive CUDA decode kernel (global loads)",
            ),
        ),
        optimized_phases=(
            _phase(
                "moe_cuda_router_topk",
                "labs.moe_cuda.optimized_router",
                "labs/moe_cuda",
                "Top-k adaptive router with capacity factor",
            ),
            _phase(
                "moe_cuda_kv_transfer_overlap",
                "labs.moe_cuda.optimized_kv_transfer",
                "labs/moe_cuda",
                "Overlapped KV transfer using CUDA streams",
            ),
            _phase(
                "moe_cuda_decode_attention_flash",
                "labs.moe_cuda.optimized_decode_attention",
                "labs/moe_cuda",
                "Flash-style decode attention",
            ),
            _phase(
                "moe_cuda_decode_kernel_tma",
                "labs.moe_cuda.optimized_decode_kernel",
                "labs/moe_cuda",
                "Blackwell TMA double-buffered decode kernel (requires Hopper/Blackwell)",
            ),
        ),
    ),
}
...
def get_scenario_spec(key: str) -> ScenarioSpec:
    try:
        ...
        raise KeyError(f"Unknown lab scenario '{key}'") from exc
...
def _load_benchmark(module_path: str):
    try:
        ...
        raise RuntimeError(f"SKIPPED: Module {module_path} not found on this hardware/stack.") from exc
...
class CapstoneScenarioBenchmark(BaseBenchmark):
    ...
    def _run_phase(self, spec: PhaseSpec) -> None:
        logger.info(
            "[labs/fullstack_cluster] Starting %s (%s/%s)",
            spec.name,
            spec.chapter,
            self.variant.value,
        )
        try:
            ...
            if "SKIPPED" in message or "requires" in message:
                logger.warning("[labs/fullstack_cluster] Phase %s skipped: %s", spec.name, message)
        ...
        if timing_ms is not None:
            logger.info(
                "[labs/fullstack_cluster] Completed %s in %.3f ms", spec.name, timing_ms
            )
        else:
            logger.info("[labs/fullstack_cluster] Completed %s", spec.name)
    ...
    def _log_summary(self) -> None:
        if not self._phase_results:
            logger.warning("[labs/fullstack_cluster] No phases executed for %s", self.scenario.key)
            return
        lines = [
            f"[labs/fullstack_cluster] Scenario '{self.scenario.title}' ({self.variant.value}) summary:"
        ]
        for result in self._phase_results:
