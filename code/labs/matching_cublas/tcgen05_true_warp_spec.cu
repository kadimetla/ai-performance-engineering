/**
 * Stage 9: TRUE Warp-Specialized tcgen05 GEMM
 * ===========================================
 * 
 * This kernel uses CUTLASS-style TRUE warp specialization:
 * - Warp 0: PRODUCER - issues TMA loads, runs AHEAD of consumers
 * - Warp 1-3: CONSUMERS - wait for data, execute MMA
 * 
 * Producer and Consumer run IN PARALLEL:
 *   Producer: [TMA0][TMA1][TMA2][TMA3]...    (runs 3-4 tiles ahead)
 *   Consumer:       [MMA0][MMA1][MMA2]...    (follows behind)
 * 
 * 4-stage pipeline with proper producer/consumer separation.
 */
...
constexpr int kStages = 4;  // Deep pipeline for max overlap
constexpr int kProducerWarp = 0;  // Warp 0 is producer
...
struct TrueWarpSpecSharedStorage {
  ...
  // Producer->Consumer barriers (one per stage)
  alignas(16) cute::uint64_t tma_barrier[kStages];
  // Consumer completion barrier (for MMA)  
...
gemm_true_warp_spec(ATensor mA, BTensor mB, CTensor mC, DTensor mD,
  ...
  bool is_producer = (warp_idx == kProducerWarp);
  bool is_consumer = !is_producer;  // Warps 1,2,3 are consumers (but only warp 1 does MMA for tcgen05)
  bool is_mma_warp = (warp_idx == 1);  // Only warp 1 issues MMA
  bool elect_one_thr = (lane_idx == 0);
  ...
  // TMEM allocation by MMA warp only
  ...
  // Initialize barriers (producer warp only)
  if (is_producer && elect_one_thr) {
  ...
  int producer_tma_phase[kStages] = {0, 0, 0, 0};
  int consumer_tma_phase[kStages] = {0, 0, 0, 0};
  int mma_phase = 0;

  tiled_mma.accumulate_ = UMMA::ScaleOut::Zero;

  // Lambda to issue TMA for a given stage and k_tile
  ...
  // ==========================================================================
  // PRODUCER: Fill pipeline and keep running in parallel with consumer
  // ==========================================================================
  // Producer must wait for consumer to release a stage before reusing it!
  // This is a simpler approach: producer and consumer interleaved in same loop
  // ==========================================================================
  
  // For tcgen05, both producer and consumer operations must be on elect_one warps
  // So we interleave: issue TMA -> wait -> MMA -> wait pattern with lookahead

  // Simpler approach: single-warp doing producer work, consumer work interleaved
  // Producer fills pipeline in prologue, then each iteration:
  //   1. Wait for current stage TMA
  //   2. Issue next stage TMA (if available)  
  //   3. Do MMA
  //   4. Wait for MMA
  
  bool elect_one_sync_warp = (warp_idx == 0);
  
  // PROLOGUE: Fill pipeline (producer work)
  if (elect_one_sync_warp && elect_one_thr) {
    if (num_k_tiles > 0) issue_tma(0, 0);
    if (num_k_tiles > 1) issue_tma(1, 1);
    if (num_k_tiles > 2) issue_tma(2, 2);
  }
  __syncthreads();  // Ensure prologue complete before mainloop

  // MAINLOOP: Consumer waits for data, issues next TMA, does MMA
  if (is_mma_warp) {
    for (int k = 0; k < num_k_tiles; ++k) {
      int curr_stage = k % kStages;
      int next_k = k + 3;  // 3 stages ahead (prologue fills 0,1,2)
      int next_stage = next_k % kStages;
      
      // Wait for current stage data from producer
      cute::wait_barrier(storage.tma_barrier[curr_stage], consumer_tma_phase[curr_stage]);
      consumer_tma_phase[curr_stage] ^= 1;

      // Issue TMA for tile k+3 (overlapped with MMA below)
      if (next_k < num_k_tiles && elect_one_thr) {
        issue_tma(next_stage, next_k);
      }

      // Execute MMA for current stage
      auto& tCrA = (curr_stage == 0) ? tCrA_0 : (curr_stage == 1) ? tCrA_1 : (curr_stage == 2) ? tCrA_2 : tCrA_3;
      auto& tCrB = (curr_stage == 0) ? tCrB_0 : (curr_stage == 1) ? tCrB_1 : (curr_stage == 2) ? tCrB_2 : tCrB_3;
...
torch::Tensor run_true_warp_spec_matmul(torch::Tensor a, torch::Tensor b) {
  ...
  auto* kernel_ptr = &gemm_true_warp_spec<
...
torch::Tensor matmul_tcgen05_true_warp_spec(torch::Tensor a, torch::Tensor b) {
  return true_warp_spec_impl::run_true_warp_spec_matmul(a, b);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("matmul_tcgen05_true_warp_spec", &matmul_tcgen05_true_warp_spec);
}
