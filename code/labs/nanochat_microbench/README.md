# Lab - NanoChat Microbench (formerly fast_nanochat)
...
Lives under `labs/nanochat_microbench`; target labels still use the `fast_nanochat` prefix for backward compatibility.
...
python -m cli.aisp bench list-targets --chapter labs/nanochat_microbench
python -m cli.aisp bench run --targets labs/nanochat_microbench --profile none
```
...
- Pass extra flags via `--target-extra-arg labs/nanochat_microbench:<target>="--flag value"`.
