...
- Single-GPU FSDP/FSDP2 benchmarks default to `labs/train_distributed/data/tinystories_packed_seq16.jsonl` plus `labs/train_distributed/data/tinyllama_min_config.json`. Multi-GPU uses `labs/train_distributed/data/tinystories_packed_seq1024.jsonl` with `labs/train_distributed/data/tinyllama_bench_config.json`. Override with `AISP_TINYSTORIES_PACKED_PATH`, `AISP_TINYSTORIES_LOCAL_PATH`, or `AISP_TINYSTORIES_CONFIG_PATH`.
- Scale up by swapping to a larger config (for example `labs/train_distributed/data/tinyllama_bench_config.json`) and pairing it with a packed dataset that matches the new sequence length.
