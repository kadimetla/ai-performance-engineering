...
## Summary
Lightweight synthetic corpora used for fast accuracy and perplexity checks without pulling external datasets. Generated by `tools/utilities/create_eval_datasets.py`.
...
- Regenerate with custom sizes:
  ```bash
  python tools/utilities/create_eval_datasets.py --output-dir eval_datasets --num-tokens {args.num_tokens} --vocab-size {args.vocab_size}
  ```
...
- `tools/utilities/compare_precision_accuracy.py` completes against each file when the harness is installed.
